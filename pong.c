...
int move_rocket(int pos, char c) {
    ...
    } else if (c == 'Z' || c == 'M') {  // заменил символы x и , на Z и M, иначе неоднозначное поведение
        if (pos != 23) {
            r = pos + 1;
        }
    }
...
void logic() {
    ...
    while (scanf("%c", &input) != 1 ||
           (input != 'A' && input != 'Z' &&
            input != ' '));  // читаем пока не введут нужный символ // пофиксил "%c\n" -> "%c\\n" из за того,
                             // что ожидалось 2 раза ввести символ
    printf("\nRight input:");
    while (scanf("%c", &input) != 1 ||
           (input != 'K' && input != 'M' &&
            input != ' '));  // пофиксил "%c\n" -> "%c\\n" из за того, что ожидалось 2 раза ввести символ
    printf("\n");
    while (1) {
        draw(l_ro, r_ro, x_ball, y_ball, l_s, r_s);
        if (turn) {
            printf("Turn Left player\n");
            while (scanf("%c", &input) != 1 ||
                   (input != 'A' && input != 'Z' &&
                    input != ' '));  // читаем пока не введут нужный символ // пофиксил "%c\n" -> "%c\\n" из
                                     // за того, что ожидалось 2 раза ввести символ
            l_ro = move_rocket(l_ro, input);
        } else {
            printf("Turn Right player\n");
            while (
                scanf("%c", &input) != 1 ||
                (input != 'K' && input != 'M' &&
                 input != ' '));  // пофиксил "%c\n" -> "%c\\n" из за того, что ожидалось 2 раза ввести символ
